...
namespace {
	...
	struct GVDiv : public ModulePass {
			...
			for (Module::global_iterator gi = M.global_begin(), ge = M.global_end();
				...
				if (gv->isConstant()) {
					...
					if (cdata) {
						...
						cur->var = dynGV;
						cur->key = RandomNumberGenerator::Generator().Random(0xff);
		...
		void addDecodeFunction(Module *mod, std::vector<encVar*> *gvars) {
			...
			BasicBlock* label_entry = BasicBlock::Create(getGlobalContext(), "entry", fdecode);
...
char GVDiv::ID = 0;
static RegisterPass<GVDiv> X("GVDiv", "Global variable (i.e., const char*) diversification pass", false, true);
